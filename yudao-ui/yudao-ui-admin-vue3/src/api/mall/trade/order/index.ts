...
// 简单订单创建请求参数
export interface TradeOrderSimpleCreateReqVO {
...
// 创建简单订单方法
export function createSimpleOrder(data: TradeOrderSimpleCreateReqVO) { 
...
export function getDraftsPage(pageNo: number, pageSize: number, customerId?: number, draftStatus?: number) {
  return request.get({ 
    ...
    params: { pageNo, pageSize, customerId, draftStatus } 
  })
}
...
export function updateDraftItem(data: any) {
  return request.post({
    url: '/trade/order/draft/item/update',
    data
  })
}
...
export function deleteDraftItem(draftId: number, itemId: number) {
  return request.delete({
    url: '/trade/order/draft/item/delete',
