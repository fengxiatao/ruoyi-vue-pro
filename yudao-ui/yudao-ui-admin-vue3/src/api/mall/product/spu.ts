...
export interface InventoryLocation {
  ...
  locationId?: number // 货位ID
  locationName?: string // 货位名称
  stock: number // 该位置的库存数量
  warnStock?: number // 预警库存
  maxStock?: number // 上限库存
}
...
export interface Sku {
  ...
  barCode?: string // 商品条码
  picUrl?: string // 图片地址
  stock?: number // 总库存（所有仓库库存之和）
  inventoryLocations?: InventoryLocation[] // 各仓库库存位置信息
...
// 获得商品 SKU 列表
export const getSkuPage = (params: PageParam) => {
  return request.get({ url: '/product/sku/page', params })
}

// 保存商品SPU和SKU信息
export const saveSpuWithSkus = (data: any) => {
  return request.post({ url: '/product/spu/save-with-skus', data })
}
