      ...
      <el-col :span="14">
        <el-card class="box-card" shadow="never">
          ...
          <el-table :data="formData" show-summary :summary-method="getSummaries">
            ...
            <el-table-column label="仓库位置" min-width="125">
              <template #default="{ row, $index }">
                ...
                >
                  <WarehouseLocationSelector
                    v-model:warehouseId="row.warehouseId"
                    v-model:rackId="row.rackId"
                    v-model:locationId="row.locationId"
                    label=""
                    :showRack="true"
                    :showLocation="true"
                    warehousePlaceholder="请选择仓库"
                    rackPlaceholder="请选择货架"
                    locationPlaceholder="请选择货位"
                    @change="onChangeWarehouseLocation($event, row)"
                  />
                </el-form-item>
              </template>
            </el-table-column>
...
const formRules = reactive({
  ...
  warehouseId: [{ required: true, message: '仓库位置不能为空', trigger: 'blur' }],
...
const handleSelectProduct = (product: ProductVO) => {
  ...
  const row = {
    ...
    actualCount: product.stockCount || 0, // 默认实际库存等于账面库存
    count: 0, // 默认盈亏数量为0
    totalPrice: 0, // 初始值
    remark: undefined
  }
