<template>
          ...
          <el-form-item label="关联订单" prop="orderNo">
            <el-input v-model="formData.orderNo" readonly>
              <template #append>
                <el-dropdown>
                  <el-button>
                    <Icon icon="ep:search" /> 选择 <Icon icon="ep:arrow-down" />
                  </el-button>
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item @click="openPurchaseOrderReturnEnableList">
                        <Icon icon="ep:document" /> 从采购订单选择
                      </el-dropdown-item>
                      <el-dropdown-item @click="openProductStockSelector">
                        <Icon icon="ep:box" /> 从库存中选择
                      </el-dropdown-item>
                    </el-dropdown-menu>
                  </template>
                </el-dropdown>
  ...
  <ProductStockSelector
    ref="productStockSelectorRef"
    @success="handleProductStockChange"
  />
</template>
...
import ProductStockSelector from './components/ProductStockSelector.vue'
...
/** 打开【从库存中选择商品】弹窗 */
const productStockSelectorRef = ref() // 商品库存选择器 Ref
const openProductStockSelector = () => {
  productStockSelectorRef.value.open()
}

/** 处理库存商品选择 */
const handleProductStockChange = (items) => {
  // 如果已有供应商，则检查是否匹配
  if (formData.value.supplierId) {
    // 这里可以添加供应商匹配逻辑，但目前先简化处理
  } else {
    // 如果未选择供应商，提示用户选择
  ...
  // 将选中的商品添加到退货单项
  formData.value.items = [...formData.value.items, ...items]
}
