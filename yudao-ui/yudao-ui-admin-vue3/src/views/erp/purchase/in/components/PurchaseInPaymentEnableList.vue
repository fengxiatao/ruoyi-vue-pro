  ...
  >
    ...
    <ContentWrap>
      ...
      >
        ...
        <el-table-column label="批次信息" align="center" width="120px">
          <template #default="scope">
            <el-popover
              placement="top"
              title="批次详情"
              width="300"
              trigger="hover"
              v-if="hasBatchInfo(scope.row)"
            >
              <template #reference>
                <el-button link type="primary">查看批次</el-button>
              </template>
              <div v-for="(batch, index) in getBatchList(scope.row)" :key="index" class="mb-10px">
                <div><strong>批次号:</strong> {{ batch.batchNumber }}</div>
                <div v-if="batch.productionDate"><strong>生产日期:</strong> {{ formatDate(batch.productionDate) }}</div>
                <div v-if="batch.expiryDate"><strong>过期日期:</strong> {{ formatDate(batch.expiryDate) }}</div>
                <div><strong>数量:</strong> {{ batch.count }}</div>
              </div>
            </el-popover>
            <span v-else>无批次信息</span>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
...
/** 批次相关函数 */
const hasBatchInfo = (row: PurchaseInVO): boolean => {
  if (!row || !row.items) return false
  return row.items.some(item => !!item.batchNumber)
}

const getBatchList = (row: PurchaseInVO): any[] => {
  if (!row || !row.items) return []
  
  const batchItems: any[] = []
  row.items.forEach(item => {
    if (item.inLocations) {
      item.inLocations.forEach(loc => {
        if (loc.batchNumber) {
          batchItems.push({
