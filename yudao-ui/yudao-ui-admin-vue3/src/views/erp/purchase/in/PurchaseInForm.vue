  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible" width="1440">
    ...
    >
      <el-row :gutter="20">
        ...
        <el-col :span="8">
          <el-form-item label="入库仓库" prop="defaultWarehouseId">
            <el-select
              v-model="formData.defaultWarehouseId"
              clearable
              filterable
              placeholder="请选择默认入库仓库"
              class="!w-1/1"
            >
              <el-option
...
const formData = ref<Partial<PurchaseInVO>>({
  id: undefined,
...
watch(
  ...
  (val) => {
    ...
    // 同步默认仓库到所有商品
    if (val.defaultWarehouseId && val.items) {
      val.items.forEach(item => {
        if (!item.warehouseId) {
          item.warehouseId = val.defaultWarehouseId
        }
      })
    }
  },
...
const open = async (type: string, id?: number) => {
  ...
  if (defaultWarehouse) {
    formData.value.defaultWarehouseId = defaultWarehouse.id
  }
...
const handlePurchaseOrderChange = (order: PurchaseOrderVO) => {
  ...
  const orderItems = order.items?.map(item => ({
    ...
    count: item.totalCount - item.inCount,
    orderItemId: item.id,
    id: undefined,
    inLocations: []
  })) || []
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as PurchaseInVO
